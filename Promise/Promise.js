...
Promise.prototype.then = function (onResolved, onRejected) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function handle(cb) {
      try {
        ...
        if (res instanceof Promise) { //当resolve/reject传入值是Promise,返回当前promise结果值
          // res.then(value=> resolve(value), reason=>reject(reason));//也可这样写
          res.then(resolve, reject) //简化写法 
          console.log('看一下这里返回promise参数是什么', res, resolve, reject)
        } else {
    ...
    /**
     * 为何这里会有这么对状态，难道定义then的时候不是promise状态已经改变了吗？
     * 回答：这里是为了解决promise状态已经改变了，但是then还没绑定回调函数的情况
     * 我明白了，可能只是定义了promise，但未绑定then回调函数，所以这里要判断状态。对吗？
     * 回答： 对的
    */
